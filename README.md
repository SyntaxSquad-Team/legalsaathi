...
Tesseract OCR	Latest	Tesseract Wiki
Git	Any	git-scm.com
...
POST	/api/ask	Ask a question, get cited answer
...
GET	/api/deadlines/{doc_id}	List deadlines
POST	/api/hearings	Add a hearing
GET	/api/hearings/{doc_id}	List hearings
POST	/api/cases	Create a case linking documents
POST	/api/risk-score	Compute risk score
POST	/api/similar-cases	Find similar past cases
GET	/api/export/summary/{doc_id}	Download summary PDF
...
AI Pipeline	LangChain, Pinecone, Gemini 2.0 Flash Lite, Groq LLaMA-3
...
SQLite DB (legalsaathi.db) auto-created on first run

Pinecone stores embeddings for semantic search

eCourts API key optional — mock data used if not set
